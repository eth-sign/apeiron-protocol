...
/// Note: the ERC-1492 Console and Cartridge interface is under development, be patient while we finish this approach to deliver something great to the community. There is many bugs or pseudo-code in here right now, I like to work public so visitors can see the pgrogress and visualize the 'spirit' of the EIP.
interface ERC1492 {
    ...
    /// Below this comment is a list of draft functions that 99.9% will exists, and in some
    /// cases replace the existing ones.
    
    /// gstate		Lock to change all IC parameters except 'istate' and 'ostate'. It can 
    /// be locked = '1', unlocked = '0' or unlocking = '2'.
    
    /// glockdelay	Time it takes for the gstate to be unlocked after changing the state 
    /// from '1' to '0'. Process: '0' -> '2', wait glockdelay, '2' -> '1'. '0' || '2' -> '1' 
    /// is instant, ULTRA SECURE in case of compromised. Requires the gstate  = '0' to change it.

    /// istate		Lock to receive tokens, except the iwhitelist and iblacklist. It can be 
    /// locked = '1', unlocked = '0' or unlocking = '2'.
    
    /// ilockdelay	Time it takes for the istate to be unlocked after changing the state 
    /// from '1' to '0'. Process: '0' -> '2', wait ilockdelay, '2' -> '1'. '0' || '2' -> '1' 
    /// is instant, ULTRA SECURE in case of compromised. Requires the istate = '0' to change value.
    
    /// iwhitelist	EIP-1492 SC addresses that can always send tokens to this contract 
    /// regardless of the 'istate'. Requires the 'istate  = '0' to add one address.
    
    /// iblacklist	EIP-1492 SC addresses that are always blocked from sending tokens to this 
    /// contract regardless of the 'istate'. Requires 'istate' = '0' to add one address.
    
    /// ostate	Lock to send tokens, except owhitelist and oblacklist. It can be locked = '1', 
    /// unlocked = '0' or unlocked = '2'.
    
    /// olockdelay	Time it takes to unlock 'ostate'. Process: '1' -> '2', wait olockdelay, 
    /// '2' -> '0'. '0' || '2' -> '1' is instant, ULTRA SECURE in case of compromised. Requires 
    /// the 'ostate' = '0' to change the value.
    
    /// owhitelist	EIP-1492 SC addresses to which tokens can always be sent regardless of 
    /// 'ostate'. Requires 'ostate' = '0' to add one address.
    
    /// oblacklist	Contract addresses a las que no queremos permitirnos enviar tokens. 
    /// Requires 'ostate' = '0' to add one address.    
    
    /// rmetadata	show metadata of a token
    
    /// cmetadata	modifiy some token metadata (specific, no general)
    
    /// gmetadata	change the general metadata pointer URI base de la metadata guardada por el owner
    
    /// xmetadata	if '1' the metata is resolved by 'gmetadata' construction, if '0' the URI will
    /// be resolved using each token metadata specific mapping.
    
    /// wmetadata	if '1' the emtadata will be written in the mapping, if '0' will not write the 
    /// received info metadata in the mapping. This doesn't depends on the 'xmetadata' current value.
    
    /// ownerchange 	function to preaprove and process an change in the owner address of this Console.
    
    /// cardigechange 	function to change the Cardige (execution smart contract) to use in the Console.
    
    /// pwhitelist	exthashcodes whitelisted to communicate to
    
    /// cwhitelist	cartdriges whitelisted to communicate to
