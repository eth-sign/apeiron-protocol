...
/// everything under this lines may not be edited/adapted yet, as we used ERC-721 to Draft this

A wallet/broker/auction application MUST implement the **wallet interface** if it will accept safe transfers.

```solidity
interface ERC1492TokenReceiver {
    /// @notice Handle the receipt of an NFT
    /// @dev The ERC1492 smart contract calls this function on the recipient
    ...
    /// @param _tokenId The FCS identifier which is being transferred
    /// @param _data Additional data with no specified format
    /// @return `bytes4(keccak256("onERC1492Received(address,address,uint256,bytes)"))`
    ///  unless throwing
    function onERC1492Received(address _operator, address _from, uint256 _tokenId, bytes _data) external returns(bytes4);
}
```
...
```json
{
    ...
    "properties": {
        ...
        "validator_n": {
            "type": "string",
            "description": "A signature of the _tokenId itself by third party "n" validator that is purposed for implementation validation as a trusted source of information for authenticity or certification."
        }        
    }
}
```
